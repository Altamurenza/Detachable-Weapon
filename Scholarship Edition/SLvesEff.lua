...
_G.DETACHABLE_WEAPON_MAIN = function()
						...
						if not CONTENT[3] then
							if not CONTENT[2] then
								if PedMePlaying(gPlayer, 'DEFAULT_KEY') and GetStickValue(16, 0) == 0 and GetStickValue(17, 0) == 0 and PedIsInAreaXYZ(gPlayer, PX, PY, PZ, 1) and not PedIsValid(PedGetTargetPed(gPlayer)) then
									if shared.gDetachableWeapon.IS_PC then
										TextPrint('USE_LOOK', 0.5, 3)
									else
										TutorialShowMessage('[Crouch] Look', 500, false)
									end
	...
	while true do
			...
			if PedIsValid(PED) then
				if PED ~= gPlayer then
					...
					if PedIsDead(PED) and not shared.gDetachableWeapon.Framework.Loot[PED] then
						shared.gDetachableWeapon.Framework.Loot[PED] = {math.random(0, 100) < ({[0] = 100, [1] = 70, [2] = 50, [3] = 40, [4] = 40, [5] = 50, [6] = 70, [7] = 100, [8] = 100, [9] = 100, [10] = 100, [11] = 40, [12] = 0, [13] = 0})[PedGetFaction(PED)], false, false}
					end
		...
		-- MANUAL DETACH
		if not PedHasWeapon(gPlayer, -1) and PedGetWeapon(gPlayer) ~= 437 and WEAPON[PedGetWeapon(gPlayer)] and PedMePlaying(gPlayer, 'DEFAULT_KEY') and not PedIsValid(TARGET) then
